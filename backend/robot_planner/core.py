...
class RobotPlanner:
    def __init__(self):
        ...
        self.use_process_for_planning = True
    ...
    async def connect_redis(self):
        ...
        logger.info("Robot Planner connected to Redis")

    async def disconnect_redis(self):
        if self.redis_client:
            await self.redis_client.close()
            logger.info("Robot Planner disconnected from Redis")
    ...
    async def plan_path(
        ...
        await inspection_missions_collection.insert_one(mission.dict(exclude={"id"}))
        return mission
    ...
    async def start_control_loop(self):
        self.running = True
        logger.info("Robot Planner control loop started")
        ...
        while self.running:
            try:
                ...
                logger.error(f"Robot Planner control loop error: {e}")
                await asyncio.sleep(5)

        logger.info("Robot Planner control loop stopped")
    ...
    async def _create_default_topology_map(self) -> TopologyMap:
        ...
        await topology_map_collection.insert_one(topology_map.dict(exclude={"id"}))
    ...
    async def _astar_with_process(
        ...
        request_data = {
            ...
            "topology_map_data": {
                "nodes": [node.dict() for node in self.topology_map.nodes],
                "edges": [edge.dict() for edge in self.topology_map.edges]
            }
        }
        ...
        try:
            response = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: send_path_planning_request(request_data, timeout=30.0)
            )
            ...
            logger.warning(f"Process-based path planning failed, falling back to in-process: {e}")
        ...
        for attempt in range(1, settings.ROBOT_PATH_PLANNING_ATTEMPTS + 1):
            try:
                ...
                for weights in weight_adjustments:
                    try:
                        path_result = None
                        if self.use_process_for_planning and is_process_running():
                            path_result = await self._astar_with_process(
                                start_km, end_km, weights
                            )

                        if path_result:
                            path = path_result["path"]
                            total_distance = path_result["total_distance"]
                            total_energy = path_result["total_energy"]
                            total_time = path_result["total_time"]
                            safety_score = path_result["safety_score"]
                            path_score = path_result["path_score"]
                            logger.info(f"Used process-based path planning, time: {path_result.get('processing_time_ms', 0):.2f}ms")
                        else:
                            ...
                            logger.info("Used in-process path planning")
