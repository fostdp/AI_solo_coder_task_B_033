class AssetManager {
    ...
    async fetchPredictions() {
        try {
            const response = await fetch('/api/assets/predictions');
            if (response.ok) {
                const data = await response.json();
    ...
    async fetchMaintenancePlans(year, month) {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                this.maintenancePlans = data.plans || [];
    ...
    async generateMonthlyPlan() {
        if (confirm('确认生成本月维修计划？\n这将基于设备状态和寿命预测自动生成。')) {
            try {
                const response = await fetch('/api/assets/maintenance/plans/generate', {
                    method: 'POST',
    ...
    async completeTask(planId, taskId) {
        try {
            const response = await fetch(
                `/api/assets/maintenance/plans/${planId}/tasks/${taskId}/complete`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' }
                }
